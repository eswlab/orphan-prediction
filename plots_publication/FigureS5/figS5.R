...
combine_all2 <- read.table("ara11_all.txt",sep="\t",header=T,stringsAsFactors = F)
method <- colnames(combine_all2)[3:8]
p <- upset(combine_all2,method, name="Prediction Method", sort_sets= F, max_degree=5,
           ...
           base_annotations = list(
             ...
             + annotate(
               ...
               label=paste('Total:', nrow(combine_all2)),
               vjust=1, hjust=1
             )
...
png("upset/newcolor_all.png", width = 18, height = 8, units = 'in', res = 600)
p
dev.off()

p <- upset(combine_all2,method, name="Prediction Method", sort_sets= F,
           width_ratio = 0.1, stripes='white', sort_intersections = F,
           #sort_intersections_by=c('degree'), sort_intersections = 'descending', sort_sets= F,
           intersections=list(
             c('Araport11','Braker','Maker','DirInf','BIND','MIND'),
...
png("upset/newcolor_allno6.png", width = 18, height = 8, units = 'in', res = 600)
p
dev.off()
